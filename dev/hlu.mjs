...
// src/hlu.mts
import { $ as $13, cd as cd7, chalk as chalk16, fs as fs18, os as os9 } from "zx";

// src/utils/common.ts
import { $ as $8, cd as cd3, chalk as chalk9, fs as fs12, globby as globby2, path as path4 } from "zx";
...
var dirsPath = os.homedir + "/.local/share/desktop-directories";
var menusPath = os.homedir + "/.config/menus/applications-merged";
var globalAppsPath = os.homedir + "/.local/share/applications/";
...
import { fs as fs10, $ as $6 } from "zx";
var launcherRemover = async () => {
  ...
  ["1", "2"].includes(removeOption.toString()) && fs10.removeSync(
    `${scriptsPath}/${launcherInfo2.info.category}/${launcherInfo2.name}.sh`
  );
  ["1", "3"].includes(removeOption.toString()) && fs10.remove(`${appsPath}/${launcherInfo2.name}.desktop`).then(async () => await $6`update-desktop-database "${globalAppsPath}"`);
};

// src/utils/launcher/launcherGenerator.ts
import { cd as cd2, chalk as chalk8, fs as fs11, $ as $7 } from "zx";
...
Categories=HLU;Games;`
  ).then(async () => await $7`update-desktop-database "${globalAppsPath}"`);
};
    ...
    (games) => {
      ...
      return !!((_a = games == null ? void 0 : games.data[0]) == null ? void 0 : _a.id) ? fetchLauncherIcon(games.data[0].id).then((icons) => {
        ...
        if (icons.data.length > 0) {
          $7`wget ${icons.data[0].thumb} -O ${launcher.name}.ico`.then(() => {
...
var ensurePaths = async () => {
  ...
  fs12.ensureDirSync(dirsPath);
  fs12.ensureDirSync(menusPath);
  fs12.ensureDirSync(iconsPath);
  fs12.ensureDirSync(scriptsPath);
  !fs12.existsSync(`${userPath}/HLU.png`) && await $8`cd ${userPath}; wget https://raw.githubusercontent.com/sergeyhist/hlu-js/main/images/HLU.png`;
  !fs12.existsSync(`${dirsPath}/hlu.directory`) && fs12.writeFileSync(
...
Icon=${userPath}/HLU.png`
  ...
  !fs12.existsSync(`${menusPath}/hlu.menu`) && fs12.writeFileSync(
    `${menusPath}/hlu.menu`,
...
Exec=${__filename} services`
  ).then(async () => await $8`update-desktop-database "${globalAppsPath}"`);
  if (!fs12.existsSync(userPath + "/settings.json") || !fs12.existsSync(userPath + "/packages.json")) {
    cd3(userPath);
    await $8`git clone https://github.com/sergeyhist/hlu-js.git`;
...
var steamOptions = async () => {
  ...
  const sessionType = await $8`echo $XDG_SESSION_TYPE`;
  console.log(
...
import { $ as $10, chalk as chalk13, fs as fs14, path as path7 } from "zx";

// src/utils/packages.ts
import { cd as cd5, chalk as chalk12, fs as fs13, path as path6, $ as $9, os as os8 } from "zx";
...
var packageInstaller = async ({ type, pack }) => {
  ...
  let release_install = async () => {
    ...
    if ((_a = packages.release[pack].flags) == null ? void 0 : _a.includes("archive")) {
      ...
      await $9`tar -xf ${path6.basename(release)} -C ${path6.basename(
        release,
    ...
    case "git":
      if (fs13.existsSync(
        ...
        if (packages.git[pack].url_args) {
          status = await $9`git reset --hard; git submodule--helper update ${packages.git[pack].url_args}; git pull ${packages.git[pack].url}`;
        } else {
          status = await $9`git reset --hard; git pull ${packages.git[pack].url}`;
        }
        $9.verbose = false;
      ...
      break;
    case "release":
      if (fs13.existsSync(releasesPath + "/" + packages.release[pack].git_name)) {
        switch (await generalInput({
  ...
  switch (await listSelector({
    ...
    case "3":
      ...
      switch (await listSelector({
        ...
        case "1":
          await $10`WINEARCH=win32 WINEPREFIX=${prefix} ${runner} wineboot -u`;
          break;
        case "2":
          await $10`WINEPREFIX=${prefix} ${runner} wineboot -u`;
          break;
      }
      break;
    case "4":
...
import { cd as cd6, fs as fs16, $ as $11 } from "zx";
var updater = async () => {
  cd6(userPath);
  await $11`git clone https://github.com/sergeyhist/hlu-js.git`;
  fs16.copySync(
...
import { chalk as chalk15, $ as $12, fs as fs17 } from "zx";
var systemdController = async () => {
  let addService = async () => {
    ...
    let path8 = await $12`systemctl show --user -p FragmentPath ${service.name} | tr -d '\\n'`;
    if (path8.stdout == "FragmentPath=") {
      path8 = await $12`systemctl show -p FragmentPath ${service.name} | tr -d '\\n'`;
      service.type = "root";
    } else {
      service.type = "user";
    }
            ...
            case "1":
              while (service + 1 != 0) {
                ...
                for (let item of services) {
                  if (item.type == "user") {
                    status = await $12`systemctl show --user -p ActiveState ${item.name} | tr -d '\\n'`;
                  } else {
                    status = await $12`systemctl show -p ActiveState ${item.name} | tr -d '\\n'`;
                  }
                  ...
                  switch (await listSelector({
                    ...
                    case "1":
                      if (services[service].type == "user") {
                        await $12`systemctl start --user ${services[service].name}`;
                      } else {
                        await $12`systemctl start ${services[service].name}`;
                      }
                      break;
                    case "2":
                      if (services[service].type == "user") {
                        await $12`systemctl stop --user ${services[service].name}`;
                      } else {
                        await $12`systemctl stop ${services[service].name}`;
                      }
                      break;
                    case "3":
                      if (services[service].type == "user") {
                        await $12`systemctl restart --user ${services[service].name}`;
                      } else {
                        await $12`systemctl restart ${services[service].name}`;
                      }
                      break;
                    case "4":
                      if (services[service].type == "user") {
                        await $12`systemctl enable --user ${services[service].name}`;
                      } else {
                        await $12`systemctl enable ${services[service].name}`;
                      }
                      break;
                    case "5":
                      if (services[service].type == "user") {
                        await $12`systemctl disable --user ${services[service].name}`;
                      } else {
                        await $12`systemctl disable ${services[service].name}`;
                      }
                      break;
                    case "6":
                      if (services[service].type == "user") {
